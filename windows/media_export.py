...
V2 图片 AES key：
- 优先读 config.json 的 image_aes_key（可由 find_image_key_windows.py 写入）
- 再尝试 uin+wxid 派生（部分环境可用）
- XOR 可从 JPEG 尾 FF D9 反推
"""
...
def find_uin(account_dir: Path) -> Optional[int]:
    """从 Windows 微信缓存里找 uin，并用账号目录后缀校验。"""
    account_id = account_dir.name
    suffix = account_id.rsplit("_", 1)[-1] if "_" in account_id else ""
    roots = [
        account_dir / "db_storage",
        account_dir.parent.parent / "app_data",
        account_dir / "config",
        Path(os.environ.get("APPDATA", "")) / "Tencent" / "xwechat",
    ]
    seen: list[int] = []
    for root in roots:
        if not root.is_dir():
...
def load_image_keys_from_config(config_path: Path | None = None) -> tuple[Optional[bytes], Optional[int]]:
    """从 windows/config.json 读取 image_aes_key / image_xor_key。"""
    ...
    try:
        ...
        return None, None
    aes = cfg.get("image_aes_key")
    xor = cfg.get("image_xor_key")
    aes_b = None
    if isinstance(aes, str) and len(aes) >= 16:
        aes_b = aes[:16].encode("ascii")
    elif isinstance(aes, (bytes, bytearray)) and len(aes) >= 16:
        aes_b = bytes(aes[:16])
    xor_i = int(xor) & 0xFF if isinstance(xor, int) else None
    return aes_b, xor_i


def guess_xor_from_thumbnails(account_dir: Path) -> Optional[int]:
    """从 *_t.dat 缩略图尾部反推 XOR key（JPEG 尾 FF D9）。"""
    attach = account_dir / "msg" / "attach"
    if not attach.is_dir():
        return None
    votes: dict[int, int] = {}
    count = 0
    for p in attach.rglob("*_t.dat"):
        try:
            data = p.read_bytes()
        except OSError:
            continue
        if len(data) < 20 or data[:6] not in (V1_SIG, V2_SIG):
            continue
        xor_size = struct.unpack_from("<I", data, 10)[0]
        if xor_size <= 0 or xor_size > len(data):
            continue
        tail = data[-xor_size:]
        if len(tail) < 2:
            continue
        ...
        if attach_root.is_dir():
            for h in hashes:
                for pat in (f"{h}.dat", f"{h}_t.dat", f"{h}_h.dat"):
                    hits = list(attach_root.rglob(pat))
                    for p in hits:
...
class MediaExporter:
    def __init__(self, account_dir: Path, decrypted_dir: Path, out_dir: Path):
        ...
        cfg_aes, cfg_xor = load_image_keys_from_config()
        derived_aes, derived_xor = derive_image_key(account_dir)
        self.aes_key = cfg_aes or derived_aes
        self.xor_byte = cfg_xor if cfg_xor is not None else derived_xor
        if self.xor_byte is None:
            self.xor_byte = guess_xor_from_thumbnails(account_dir)
        ...
        if self.aes_key:
            ...
            print(
                ...
                "请先打开几张微信图片，再运行：python windows/find_image_key_windows.py"
            )
