# -*- coding: utf-8 -*-
"""
Windows 微信聊天记录导出 Excel（WeChat 4.x / 4.1+）
...
只导出指定联系人/群（按备注/昵称/wxid 模糊匹配，可多次）：
  ...
  python win-test.py --fresh --chat 妈 --chat 爱心小屋

首次使用前：
  1. 安装依赖：pip install -r requirements.txt
  2. 登录 Windows 微信（Weixin.exe）
  3. 以管理员身份运行：python win-test.py
"""
...
WIN_DIR = Path(__file__).resolve().parent / "windows"
...
def extract_keys(db_dir: str, force: bool = False) -> Path:
    """用 wcdb-key-tool 提取密钥（runtime Config.Cipher + 旧版内存扫描）。"""
    keys_file = WIN_DIR / "all_keys.json"
    ...
    if not _is_admin():
        ...
        print("建议：右键「以管理员身份运行」终端后再执行。\n")

    tool = WIN_DIR / "wcdb_key_tool_windows.py"
    if not tool.exists():
    ...
    print("正在提取数据库密钥（兼容微信 4.0 内存扫描 + 4.1+ Config.Cipher 扫描）...")
    print("请确认微信已登录。\n")
    ...
    try:
        subprocess.check_call(cmd, cwd=str(WIN_DIR))
    except subprocess.CalledProcessError as e:
        raise SystemExit(
            f"密钥提取失败（退出码 {e.returncode}）。"
            "请以管理员身份运行，并确认微信已登录、数据目录正确。"
        ) from e

    keys = _load_valid_keys(keys_file)
    if not keys:
        raise SystemExit("密钥提取结果为空。请加 --refresh-keys 重试。")
...
def export_chats_to_excel(
    ...
    if _zstd_dctx is None:
        print("提示：未安装 zstandard，zstd 压缩文本将跳过（pip install zstandard）")
...
def main():
    parser = argparse.ArgumentParser(description="Windows 微信聊天记录导出 Excel")
    parser.add_argument("-o", "--output", default="wechat_backup_win.xlsx", help="输出 Excel 路径")
    ...
    if not args.skip_keys:
        extract_keys(db_dir=cfg["db_dir"], force=args.refresh_keys)
    elif not _load_valid_keys(Path(cfg["keys_file"])):
        raise SystemExit(
            "没有可用密钥。请先完整跑一次：python win-test.py"
        )
